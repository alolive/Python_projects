#!/usr/bin/python
#
# For each Satellite V6 listed hosts add ansible_host
# values from ajsond bunch facts file.
# Directly fill skeleton hostvars nested JSON.
#
# For extended information, please use:
#
# /usr/bin/pydoc ./160_ansible_host_hostvars.py
#
# Changelog:
#
...
def main():
    ...
    string = "Starting hostvars feeding with ungrouped ansible_host values."
    logger.info( string )
    ...
    for host_json in hosts_list_json['results']:
      ...
      try:
        ...
        # Load Ansible_host doped bunch files as a JSON.
        infi = "%s/%s.ajsond" % ( cf.output_dir, fqdn )
        fd_ansh_json = open( infi, 'r' ) 
        ansh_json = json.loads( fd_ansh_json.read() )

        for host in ansh_json.keys():

          # Get Ansible host facts for this host.
          ansh_facts = ansh_json[host]

          # Parse facts searching for keys with value.          
          for (ik,iv) in ansh_facts.items():
        ...
        string = "Can not open %s doped Ansible host evaluated bunch file." % ( fqdn )
        logger.error ( string )
