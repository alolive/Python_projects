...
def GetJson( group_name ):
    """
    First arg is the per value Gconf group name
    ...
    try:
      ...
      infig = "%s/grp_gcf_%s.json" % (cf.input_dir , group_name )
      fd_infi = open( infig , 'r' )
      gcfsnip_json = json.loads( fd_infi.read() )
...
def create( key , group_name, json_value_specs ):
    ...
    MyGcfGroupVars = { "children" : [], \
                       "hosts" : [], \
                       "vars" : MyVars }

    # Add group name. 
    MyGcfGroup = { group_name : MyGcfGroupVars }

    # Dump to file for later use.
    dump2file ( group_name , MyGcfGroup )

    # Log Gconf group creation phase as info.
    string1 = "Gconf group snippet grp_gcf"
    string = "%s_%s.json created." % ( string1 , group_name )
    logger.info( string )

def dump2file ( group_name , json_group ):
    """
    First arg is the group name Gconf snippet file to
    ...
    oufig = "%s/grp_gcf_%s.json" % ( cf.output_dir, group_name )
...
def populate ( fqdn , gcf_snip ):
    """
    First arg is the fqdn of host to insert in group held
    in second arg as JSON gcf_snip dictionnary.
    """
    ...
    for k in gcf_snip.keys():

       group_core = gcf_snip[k]
...
def delete ():
    """
    Delete all grp_gcf.*.json files to clean place
    ...
    path = "%s/grp_gcf_*.json" % ( cf.output_dir )
    ...
    string = "All previous Gconf groups in %s removed." % ( cf.output_dir )
