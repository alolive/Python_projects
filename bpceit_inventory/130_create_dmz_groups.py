...
# values to be grouped is ejsond bunch facts file.
# Create groups and feed them.
#
# For extended information, please use:
#
# /usr/bin/pydoc ./130_create_dmz_groups.py
#
# Changelog:
#
# v 1.0 - 2020/05/27: Creation
...
def main():
    ...
    Parse the doped DMZ bunch facts file.
    ...
    # Log DMZ groups creation starting phase.
    string = "Starting global DMZ values groups creation."
    logger.info( string )
    ...
    for host_json in hosts_list_json['results']:
      ...
      try:
        ...
        # Load DMZ doped bunch files as a JSON.
        infi = "%s/%s.ejsond" % ( cf.output_dir, fqdn )
        fd_dmz_json = open( infi, 'r' ) 
        dmz_json = json.loads( fd_dmz_json.read() )

        for host in dmz_json.keys():

          dmz_facts = dmz_json[host]

          # Parse facts searching for keys with value.          
          for (ik,iv) in dmz_facts.items():
              ...
              if ( not sk ):
                ...
                if ( found == 1 ):
                  ...
                  if ( value_specifications['built-in-group'] == "yes" ):
                    ...
                    ( rc , dmz_snip ) = dmz_group.GetJson ( iv )

                    # Check if we need to create snippet before feeding.                    
                    if ( rc == 0 ):

                      dmz_group.create( ik, iv , value_specifications )
                      ( rc , dmz_snip ) = dmz_group.GetJson ( iv )

                    # Add current fqdn to snippet hosts list.
                    dmz_group.populate( fqdn, dmz_snip )
                    
                    # Save new member to the snippet file.
                    dmz_group.dump2file( iv, dmz_snip )

      except:

        # Log an error as we can not open a ejsond file.
        string = "Can not open %s doped DMZ bunch file." % ( fqdn )
        logger.error ( string )
